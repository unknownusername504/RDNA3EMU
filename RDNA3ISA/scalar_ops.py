...
class ScalarOps:
    ...
    def s_add_u32(self, reg_d, reg_s0, reg_s1):
        reg_s0_value = self.register_file.get_sgpr_u32(reg_s0)
        reg_s1_value = self.register_file.get_sgpr_u32(reg_s1)
        reg_d_value = reg_s0_value + reg_s1_value
        reg_scc_value = 1 if (reg_d_value >= 2**32) else 0
        self.register_file.set_scc_value(reg_scc_value)
        self.register_file.set_sgpr_u32(reg_d, reg_d_value)
    ...
    def s_sub_u32(self, reg_d, reg_s0, reg_s1):
        ...
        self.register_file.set_scc_value(reg_scc_value)
        self.register_file.get_sgpr_u32(reg_d, reg_d_value)
    ...
    def s_add_i32(self, reg_d, reg_s0, reg_s1):
        reg_s0_value = self.register_file.get_sgpr_i32(reg_s0)
        reg_s1_value = self.register_file.get_sgpr_i32(reg_s1)
        reg_d_value = reg_s0_value + reg_s1_value
        sign = 1 if (reg_d_value < 0) else 0
        reg_scc_value = 1 if (sign != (reg_s0_value < 0)) else 0
        self.register_file.set_scc_value(reg_scc_value)
        self.register_file.set_sgpr_i32(reg_d, reg_d_value)
    ...
    def s_addc_u32(self, reg_d, reg_s0, reg_s1):
        reg_s0_value = self.register_file.get_sgpr_u32(reg_s0)
        reg_s1_value = self.register_file.get_sgpr_u32(reg_s1)
        reg_scc_value = self.register_file.get_scc_value()
    ...
    def s_lshl_b64(self, reg_d, reg_s0, reg_s1):
        reg_s0_value = self.register_file.get_sgpr_u64(reg_s0)
        reg_s1_value = self.register_file.get_sgpr_u64(reg_s1)
        reg_d_value = reg_s0_value << reg_s1_value
        reg_scc_value = 1 if (reg_d_value != 0) else 0
        self.register_file.set_scc_value(reg_scc_value)
        self.register_file.set_sgpr_u64(reg_d, reg_d_value)
    ...
    def s_ashr_i64(self, reg_d, reg_s0, reg_s1):
        reg_s0_value = self.register_file.get_sgpr_i64(reg_s0)
        reg_s1_value = self.register_file.get_sgpr_u64(reg_s1)
        reg_d_value = reg_s0_value >> reg_s1_value
        reg_scc_value = 1 if (reg_d_value != 0) else 0
        self.register_file.set_scc_value(reg_scc_value)
        self.register_file.set_sgpr_i64(reg_d, reg_d_value)
    ...
    def s_or_not1_b64(self, reg_d, reg_s0, reg_s1):
        reg_s0_value = self.register_file.get_sgpr_i64(reg_s0)
        reg_s1_value = self.register_file.get_sgpr_i64(reg_s1)
        reg_d_value = reg_s0_value | ~reg_s1_value
        reg_scc_value = 1 if (reg_d_value != -1) else 0
        self.register_file.set_scc_value(reg_scc_value)
        self.register_file.set_sgpr_u64(reg_d, reg_d_value)
    ...
    def s_bfe_i32(self, reg_d, reg_s0, reg_s1):
        ...
        if (reg_d_value >> (field_size - 1)) == 1:
            reg_d_value |= ~((1 << field_size) - 1)
        reg_scc_value = 1 if (reg_d_value != 0) else 0
        self.register_file.set_scc_value(reg_scc_value)
        self.register_file.set_sgpr_i32(reg_d, reg_d_value)
    ...
    def s_pack_ll_b32_b16(self, reg_d, reg_s0, reg_s1):
        reg_s0_value = self.register_file.get_sgpr_u16(reg_s0)
        reg_s1_value = self.register_file.get_sgpr_u16(reg_s1)
    ...
    def s_move_b32(self, reg_d, reg_s0):
        reg_s0_val = self.register_file.get_register_value(reg_s0, signed=False, size=32)
        self.register_file.set_register_value(reg_d, reg_s0_val, signed=False, size=32)

    # Move scalar input into a scalar register. (64-bit)
    def s_move_b64(self, reg_d, reg_s0):
        reg_s0_val = self.register_file.get_register_value(reg_s0, signed=False, size=64)
        self.register_file.set_register_value(reg_d, reg_s0_val, signed=False, size=64)

    # Move scalar input into a scalar register iff SCC is nonzero.
    def c_move_b32(self, reg_d, reg_s0):
       if (self.register_file.get_scc_value() == 1):
    ...
    def s_brev_b32(self, reg_d, reg_s0):
        ...
        self.register_file.set_register_value(reg_d, tmp, signed=False, size=32)

    # Reverse the order of bits in a scalar input and store the result into a scalar register.
    def s_brev_b64(self, reg_d, reg_s0):
        reg_s0_val = self.register_file.get_register_value(reg_s0, signed=False, size=64)
        tmp = utils.rev_b64(reg_s0_val)
        self.register_file.set_register_value(reg_d, tmp, signed=False, size=64)
    ...
    def s_sext_i32_i8(self, reg_d, reg_s0):
        reg_s0_val = self.register_file.get_register_value(reg_s0, signed=True, size=8) 
        tmp = utils.sext_i32(reg_s0_val, 8) 
        self.register_file.set_register_value(reg_d, tmp, signed=True, size=32)

    # Sign extend an 16-bit scalar value to 32-bits
    def s_sext_i32_i16(self, reg_d, reg_s0):
        reg_s0_val = self.register_file.get_register_value(reg_s0, signed=True, size=16) 
        tmp = utils.sext_i32(reg_s0_val, 16) 
        self.register_file.set_register_value(reg_d, tmp, signed=True, size=32)

    # Set bit to 0 at offset (reg_s0) in reg_d
    def s_bitset0_b32(self, reg_d, reg_s0):
        offset = self.register_file.get_register_value(reg_s0, signed=False, size=5) 
        reg_d_val = self.register_file.get_register_value(reg_d, signed=False, size=32) 
        tmp = utils.bitset0(reg_d_val, offset) 
        self.register_file.set_register_value(reg_d, tmp, signed=False, size=32)

    def s_bitset0_b64(self, reg_d, reg_s0):
        offset = self.register_file.get_register_value(reg_s0, signed=False, size=6) 
        reg_d_val = self.register_file.get_register_value(reg_d, signed=False, size=64) 
        tmp = utils.bitset0(reg_d_val, offset) 
        self.register_file.set_register_value(reg_d, tmp, signed=False, size=64)
    ...
    def s_abs_i32(self, reg_d, reg_s0):
        reg_s0_val = self.register_file.get_register_value(reg_s0, signed=True, size=32) 
        tmp = abs(reg_s0_val) 
        self.register_file.set_register_value(reg_d, tmp, signed=True, size=32)
